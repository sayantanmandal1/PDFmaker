...
export function ErrorMessage({ message, title = 'Error', onRetry, className = '' }: ErrorMessageProps) {
  return (
    <div className={`bg-red-50 border border-red-200 rounded-lg p-4 sm:p-6 ${className}`} role="alert">
...
export function EmptyState({ title, message, icon, action, className = '' }: EmptyStateProps) {
  return (
    <div className={`text-center py-12 px-4 ${className}`}>
      ...
      <p className="text-sm sm:text-base text-gray-600 mb-6 max-w-md mx-auto">{message}</p>
      {action && (
        <button
